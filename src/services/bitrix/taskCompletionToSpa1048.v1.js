...
function pruneProcessedTasks(ttlMs) {
  const now = Date.now();
  for (const [taskId, ts] of processedTasks.entries()) {
    if (now - ts > ttlMs) processedTasks.delete(taskId);
  }
}
...
function parseTaskCrmBindings(task) {
  ...
  for (const raw of bindings) {
    ...
    if (!text) continue;
    const matchT = /^T(\d+)_(\d+)$/i.exec(text);
    if (matchT) {
      itemId = parseNumber(matchT[2]);
      matched = text;
      matchers.push({ raw: text, match: matchT[0], itemId });
      break;
    }
    const matchD = /^D[_:|-]?(\d+)[_:|-](\d+)$/i.exec(text);
    if (matchD) {
      itemId = parseNumber(matchD[2]);
      matched = text;
      matchers.push({ raw: text, match: matchD[0], itemId });
      break;
    }
...
async function pollRecentlyCompletedTasks() {
  ...
  pruneProcessedTasks(ttlMs);

  console.log('[task-event] poll_start', { windowSec, limit, sinceIso });

  const result = await bitrix.call('tasks.task.list', {
    order: { CHANGED_DATE: 'desc' },
    filter: {
      STATUS: COMPLETED_STATUS,
      '>=CHANGED_DATE': sinceIso,
    },
    select: ['ID', 'STATUS', 'UF_CRM_TASK', 'TITLE', 'CLOSED_DATE', 'CHANGED_DATE'],
    start: 0,
  });

  const tasks = result?.tasks || result?.result?.tasks || result?.result || [];
  const list = Array.isArray(tasks) ? tasks.slice(0, limit) : [];
  ...
  for (const task of list) {
    const taskId = parseNumber(task?.id || task?.ID);
    if (!taskId) continue;
    if (processedTasks.has(taskId)) continue;

    const bindingInfo = parseTaskCrmBindings(task);
    if (!bindingInfo.itemId) {
      processedTasks.set(taskId, Date.now());
      continue;
    }

    await updateSpaStage({
      itemId: bindingInfo.itemId,
      stageId,
      entityTypeId,
    });
  ...
  console.log('[task-event] poll_end', {
    candidates: list.length,
    processed,
  });

  return { processed, candidates: list.length };
}
...
async function handleTaskCompletionEvent(req, res) {
  try {
    ...
    if (!taskId) {
      const pollResult = await pollRecentlyCompletedTasks();
      return res.json({
        ok: true,
        action: 'polled',
        processed: pollResult.processed,
        candidates: pollResult.candidates,
        debug: true,
      });
    }
